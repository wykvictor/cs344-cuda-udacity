...
int main(int argc, char **argv) {
  ...
  cv::Mat src = cv::imread("IMAG0179_small.jpg", cv::IMREAD_GRAYSCALE);

  if (!src.data) {
    printf("failed opening jpg\n");
    exit(1);
  }
  ...
  GpuMat d_lines;
  {
    ...
    Ptr<cuda::HoughSegmentDetector> hough = cuda::createHoughSegmentDetector(1.0f, (float)(CV_PI / 180.0f), 60, 5);
